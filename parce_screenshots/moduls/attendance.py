...
from parce_screenshots.moduls.locators import ATTENDANCE_LOCATOR, INCORRECT_DATA_SELECTOR, ACTIVATION_REQUIRES_SELECTOR
        ...
        for attempt in range(attempts):
            ...
            if "Attention! For this report you need an additional activation." in page_content:
                ...
                try:
                    ...
                    if element:
                        path = f'{SCREENSHOTS_DIR}/{hotel_title or "default"}/04_attendance.png'
            ...
            await element.screenshot(path=f'{SCREENSHOTS_DIR}/{hotel_title or "default"}/04_attendance.png')
            return

        # Ошибка "неверные данные" осталась после всех попыток
        if await page.is_visible(INCORRECT_DATA_SELECTOR):
            error_element = await page.query_selector(INCORRECT_DATA_SELECTOR)
            await error_element.screenshot(path=f'{SCREENSHOTS_DIR}/{hotel_title or "default"}/04_attendance.png')
