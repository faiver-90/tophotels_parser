...
async def top_screen(page: Page, hotel_id, hotel_title=None):
    try:
        ...
        await element.screenshot(path=f"{SCREENSHOTS_DIR}/{hotel_title}/01_top_element.png")
        element2 = await page.query_selector(POPULARS_LOCATOR)
        await element2.screenshot(path=f"{SCREENSHOTS_DIR}/{hotel_title}/02_populars_element.png")
