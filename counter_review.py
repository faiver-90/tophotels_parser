...
from config_app import OUTPUT_FILE_COUNTER_REVIEW, HOTELS_IDS_FILE
...
class HotelParser:
    ...
    async def get_hotel_info(self, hotel_id: str) -> Tuple[str, str, int]:
        ...
        try:
            ...
            questions_text = await self._get_text_or_default(
                "#container > article > h2", ""
            )
            count = (
                int("".join(filter(str.isdigit, questions_text)))
                if questions_text
                else 0
            )
    ...
    async def get_ratings(
        ...
        try:
            await self.page.wait_for_selector(
                ".card-hotel-rating-statistic", timeout=10000
            )
            blocks = await self.page.query_selector_all(
                ".card-hotel-rating-statistic .card-hotel-rating"
            )
...
class ExcelExporter:
    def __init__(self, file_path: str):
        ...
        self.sheet.append(
            ["ID", "Название", "Количество вопросов", "Общий рейтинг", "Рейтинг 2025"]
        )
...
class HotelScraperApp:
    ...
    async def run(self):
        async with async_playwright() as p:
            ...
            exporter = ExcelExporter(OUTPUT_FILE_COUNTER_REVIEW)
            ...
            for hotel_id in self.hotel_ids:
                ...
                print(
                    f"al{hotel_id}: {title} — {question_count} | общий: {overall}, 2025: {y2025}"
                )
...
if __name__ == "__main__":
    ids = load_hotel_ids(HOTELS_IDS_FILE)
    print(ids)
